...
#    score_index = [0, 1, 1, 1, 50, 10, 10, 60, 1, 1]
    score_index_0 = [0, 1, 1, 2, 50, 8, 9, 60, 1, 1]
    ...
    if robot.cargo_type != "0":
        ...
        for sellable_areas in sellable_areas_matrix:
            for sellable_area in sellable_areas:
                # 在所有工作台中先找1-4号，7号工作台,添加到sellable_areas_result中
                worktable_type = int(sellable_area.type)
                if worktable_type == 7 or worktable_type < 5:
                    sellable_areas_result.append(sellable_area)
                # 在所有工作台中找5-6号工作台,如果y轴坐标等于26.25，添加到sellable_areas_result中
                position = float(sellable_area.location[1])
                if position == 26.25:
                    sellable_areas_result.append(sellable_area)
...
def calc_priority(robot, target: Worktable, distance):
    ...
    # 机器人携带物品的收益
    if robot.cargo_type != "0":
        if robot.cargo_type == "4":
            return 1000000000
        if robot.cargo_type == "7":
...
def choose_target(robot, sellable_areas_result: List[Worktable]) -> Worktable:
    ...
    """
    # 选择距离最近的工作台
    for i, worktable in enumerate(sellable_areas_result):
        if worktable.type == "7":
            return worktable
        if worktable.type == "4":
            return worktable
