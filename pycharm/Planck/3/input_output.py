...
def read_frame():
    ...
    for i in range(4):
        ...
        robot = Robot(robot_data[0], robot_data[1], robot_data[2], robot_data[3], robot_data[4],
                      (robot_data[5], robot_data[6]), robot_data[7], (robot_data[8], robot_data[9]), radius)
        if robot_data[1] != "0":
            # 设置需求数目
        ...
        robots.append(robot)
    input()  # 读取 OK
    #sys.stderr.write(str(material_demand_num_list))
    #sys.stderr.write("\n")
    #exit(0)
    return frame_no, money, worktable_num, work_tables, robots, material_demand_num_list, nums_of_products
