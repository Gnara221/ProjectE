...
"""def search_scope(state,robotid,search_ran):
    #机器人状态
    robot = robotid
    #机器人搜索上下限
    search_lower_limit =  state.robots(robot.position) + range
    search_upper_limit =  state.robots(robot.position) - range
    ...
    if robot.cargo_type != '0':
        worktables = [state.worktables[str(sellable_worktables)] for sellable_worktables in sell_info[robot.cargo_type]]
        #找到最近的工作台
        sellable_areas_result = []

        for sellable_areas in worktables:
            if sellable_areas[float(sell_info[1])] <
        nearest_sellable_worktable = Worktable(state.worktables,)"""
