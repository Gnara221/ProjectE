...
#define Moto_PwmMax 720
int16_t MOTO1_PWM = 0;       //PA6
int16_t MOTO2_PWM = 0;    	 //PA7
int16_t MOTO3_PWM = 0;    	 //PB0
int16_t MOTO4_PWM = 0;    	 //PA1


/******************************************************************************************
* 函  数：void MOTOR_Init(void)
* 功  能：电机引脚初始化 以及TIM3 配置输出PWM TIM3有4个捕获/输出通道
* 参  数：无
* 返回值：无
...
void MOTOR_Init(void)
{
	...
	TIM_TimeBaseInitStruct.TIM_Period=1000-1;   	  //设置自动重装载的周期值    周期！！！
	...
	TIM_TimeBaseInit(TIM3,&TIM_TimeBaseInitStruct);   		   //定时器初始化函数 TIM3
	
	//72M/100=720Khz的计数频率,重装载值1000，所以PWM频率为 720KHz/1000=720hz.    
	
	TIM_OCInitStruct.TIM_OCMode=TIM_OCMode_PWM1;   //PWM模式1		
	TIM_OCInitStruct.TIM_Pulse=0;   						   //初始化占空比为0 写CCRx的值
	...
	//  高电平有效、PWM模式1、向上计数 当计数器的值小于CCRx时有效、有效为高电平、占空比为周期前一部分
