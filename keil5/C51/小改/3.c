...
/*		这里针对上面的while循环做一点优化，仅做参考			*/
/*			因为是一次性使用，所以这里也针对设计				*/


	//开机后检测不到物体时，将一直卡在这里
	while(IR_IN == 1);
	/*这里进判断，因为上面循环已经跳出，所以这里IR_IN必定为0，所以if(IR_IN == 0)判断可要可不要*/
	if(IR_IN == 0)
	{
		/*抬杆标志置1*/
		flag = 1;
	...
	/*这里的逻辑是，检测到物体时一直卡着，直到检测不到物体后跳出*/
	while(IR_IN == 0);
	/*进判断，如果IR_IN和flag同时为1，则落杆，flag置0，这里的判断同样可要可不要*/
	if(IR_IN == 1 && flag == 1)
	{
		ServoPwmDat = 500;
		flag = 0;
...
/*		由于考虑到是手动开启，所以这里加入手动控制的逻辑，		*/
/*		 具体实现还需要根据手动控制的信息传入方式来进行。		*/
/*	因为看代码只知道是通过串口通信的方式在进行控制信号的传入，	*/
/*	 所以并不知道传入值的具体意义，
/*	这里拟定一个变量pc_data,为0时表示接收到控制信号，为1则相反	*/	
...
/*		这里的else为上面IR_IN不为0的情况，即没检测到物体时		*/
/*				如果接收到控制信号，就落杆。					*/
		else
		{
			if(pc_data == 1)
